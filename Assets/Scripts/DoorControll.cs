...
public class DoorControll : MonoBehaviour
{
    ...
    [Header("오브젝트 교체")]
    public GameObject lockAndKeyGroup;
    public GameObject unlockAndKeyGroup;

    [Header("대사 설정")]
    [TextArea] public string lockedMessage = "열리지 않는다... 열쇠만 있으면 창틀에 손을 넣어서 열 수 있을텐데...";
    [TextArea] public string openMessage = "문이 열렸다. 주변에 필요한 물품부터 찾아보자.";
    ...
    void TryOpenDoor()
    {
        ...
        if (hasKey)
        {
            ...
            if (lockAndKeyGroup) lockAndKeyGroup.SetActive(false);
            if (unlockAndKeyGroup) unlockAndKeyGroup.SetActive(true);
